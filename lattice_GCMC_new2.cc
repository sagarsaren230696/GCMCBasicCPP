...
const bool write_output_config = true;
...
int main(int argc, char *argv[])  {
    ...
    if (write_to_file) {
        ...
        if (! GG) 
            sprintf(outputfilename, "results_lattice_new/SIM_UnoGG%.2f_d%.2f.txt", U_0, d_sites);
        if (GG) 
            sprintf(outputfilename, "results_lattice_new/SIM_U%.2f_d%.2f.txt", U_0, d_sites);
        ...
        fprintf(outfile, "Pressure(bar),Loading(vSTP/v),Loading(Molecules_per_UC),LangmuirLoading(Molecules_per_UC)\n");
    }
    ...
    for (int i_P = 0; i_P < pressures.size(); i_P++) {
        ...
        if (write_output_config){
            char outputConfigFileName[1024];
            sprintf(outputConfigFileName, "results_lattice_new/lattic_config_U%.2f_d%.2f_P%.2f.txt", U_0, d_sites,P);
            configFile = fopen(outputConfigFileName, "w");
            if (configFile==NULL) {
                printf("ERROR: could not open output file with name %s\n", configFile);
                exit(EXIT_FAILURE);
            }
            fprintf(configFile,"x,y,z,occ\n");
        }
        ...
        if (write_to_file) {
            double langmuirK = site_volume * exp(-U_0 / T) / (kb * T);
            fprintf(outfile, "%f,%.3f,%.3f,%.3f\n", pressures[i_P], vSTPv, N_avg, N_sites * langmuirK * P / (1.0 + langmuirK * P));
        }

        if (write_output_config == true){
            for (int i = 0; i < N_sites; i++) {
                int occ = 0;
                if (lattice_sites[i].occ == true)
                    occ = 1;
                fprintf(configFile,"%f,%f,%f,%d\n",lattice_sites[i].x,lattice_sites[i].y,lattice_sites[i].z,occ);
            }
            fclose(configFile);
        }
    } // end pressure loop
