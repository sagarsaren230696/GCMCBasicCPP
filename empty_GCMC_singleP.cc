...
int main(int argc, char *argv[]) {
    ...
    // store N_avg methane at pressure P here (the isotherm!) deleted another line
    

    // set up output file
    char outputname[1024];
    sprintf(outputname, "./results_empty_box/SIM_U%.2f_P%.2f.txt", U_0,fug);
    FILE *outfile;
    if (write_to_file) { 
    ...
    // fprintf(equilibrationDataFile, "CycleNo,Navg\n");
        ...
        for (int i = 0; i < N_inner_cycles; i++) { // The MC moves will be applied to the number of existing molecules in the system at most
            ...
            if (which_move == 0) {
                ...
                if (uniform01(generator) < acceptance_insertion) { 
                    ...
                    N_ch4++;
                    N_accept++;
                    // fprintf(equilibrationDataFile,"%d,%f\n",MC_counter,dE);
                } 
                else {
