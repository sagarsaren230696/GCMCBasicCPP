...
Q0 = np.linspace(4,16,5) # Heat of adsorption in kJ/mol
U0 = -Q0/kb/N*1000
print(U0)
...
for i,u in enumerate(U0):
    ...
    # sp.call([r"./lattice_GCMC.exe","{}".format(u),"{}".format(200),"{}".format(8.0),"{}".format(0)])
    # dataFile = pd.read_csv(r"./results_lattice_model/SIM_UnoGG{:.2f}_d{:.2f}.txt".format(u,8.0),sep=',',header=3)
    # sp.call([r"./lattice_GCMC.exe","{}".format(u),"{}".format(200),"{}".format(8.0),"{}".format(1)])
    dataFile = pd.read_csv(r"./results_lattice_model/SIM_U{:.2f}_d{:.2f}.txt".format(u,8.0),sep=',',header=3)
    ax.plot(dataFile["Pressure(bar)"].values*1e-3,dataFile["Loading(vSTP/v)"].values,color=colors[i],label="{} kJ/mol".format(Q0[i]))
df = pd.read_csv("ExpUptakeData.csv")
ax.scatter(dataFile["Pressure(bar)"].values*1e-3,df["Vol. Uptake"],s=50,marker="^")
ax.set_ylim(bottom=0)
ax.set_xlim([0,10.0])
ax.set_xlabel("Pressure (kPa)")
ax.set_ylabel("Methane uptake (cc(STP)/cc)")
ax.legend(loc="best")
plt.savefig(r"./results_lattice_model/uptake_profile_d8.0_GG.png",dpi=300)
